# -*- coding: utf-8 -*-
"""chunk014 通用渲染引擎 —— 语文 / 生物 9 页学生版课堂 PPT。
...
用法（每个 tools/_render_<id>.py）：
    from _kit_chunk014 import build, derive_reflections, derive_lead, derive_summary
    d = json.load(open(...))
    sources = ["来源：……", "来源：……"]
...
def s_cover(s, d, lead):
    ...
    lt = d.get('lessonTypeName', '').replace('授课', '')
    textbox(s, M + Inches(0.15), H - Inches(0.74), Inches(11.5), Inches(0.45),
            [{'text': f"第{d.get('periodNumber', '?')}课时 · {lt} · 学生课堂版",
