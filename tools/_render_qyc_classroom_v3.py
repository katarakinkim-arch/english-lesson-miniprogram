...
OUT = os.path.join(PROJ_DIR, 'cn_qyc_classroom_v3')
IMG_DIR = os.path.join(PROJ_DIR, 'classroom_images')
...
def place_photo(img, d, photo, x, y, w, h, caption=None):
    if photo:
        cropped = crop_to_fill(photo, w, h)
        img.paste(cropped, (x, y))
        d.rectangle([(x, y), (x + w, y + h)], outline=BORDER, width=2)
        if caption:
            d.rectangle([(x, y + h - 28), (x + w, y + h)], fill=(0, 0, 0, 180))
            d.text((x + 10, y + h - 24), caption, fill=WHITE, font=F_SMALL)
    else:
        d.rectangle([(x, y), (x + w, y + h)], fill=LIGHT_BG, outline=LIGHT_GRAY)
        d.text((x + w//2 - 40, y + h//2 - 10), '[图片]', fill=GRAY, font=F_BODY)
...
def slide_cover():
    """1. 封面 — 秋景照片+标题"""
    img, d = new_slide()
    photo = load_img('00_cover.jpg')
    if photo:
        cropped = crop_to_fill(photo, W, H)
        img.paste(cropped, (0, 0))
        # 暗色叠层
        overlay = Image.new('RGB', (W, H), INK)
        blended = Image.blend(cropped, overlay, 0.45)
        img.paste(blended, (0, 0))
        d = ImageDraw.Draw(img)
    else:
        d.rectangle([(0, 0), (W, H)], fill=INK)
    # 标题
    d.text((MARGIN, 120), '沁园春·长沙', fill=WHITE, font=load_font('msyhbd.ttc', 72))
    d.text((MARGIN + d.textlength('沁园春·长沙', load_font('msyhbd.ttc', 72)) + 30, 140), '毛泽东', fill=(200,210,220), font=F_TITLE)
    # 色条
    d.rectangle([(MARGIN, 216), (MARGIN + 200, 220)], fill=FROST)
    d.rectangle([(MARGIN, 230), (MARGIN + 160, 268)], fill=FROST)
    d.text((MARGIN + 20, 238), '第一课时', fill=WHITE, font=F_BODY_B)
    # 底部信息
    d.text((MARGIN, 380), '统编版高中语文必修上册 · 第一单元', fill=(200,200,200), font=F_BODY)
    d.text((MARGIN, 420), '知人论世 · 诵读感知 · 意象品析', fill=(180,190,200), font=F_SUB)
    d.text((MARGIN, H - 50), '课堂讲授版', fill=(150,160,170), font=F_SMALL)
...
def slide_leadin_autumn():
    ...
    y += 30
    d.text((MARGIN + 10, y), '  刘禹锡《秋词》', fill=XIANG, font=F_BODY_B)
    y += 24
    d.text((MARGIN + 10, y), '  "自古逢秋悲寂寥，我言秋日胜春朝"', fill=INK, font=F_BODY)
    y += 38
...
def slide_author():
    ...
    for q in quotes:
        ...
        y += 26
    y += 16
    d.text((MARGIN, y), '1925年秋，32岁的毛泽东重游', fill=INK, font=F_BODY)
    y += 24
    d.text((MARGIN, y), '橘子洲，写下这首词。', fill=INK, font=F_BODY)
    # 右侧：橘子洲实景照片
    photo = load_img('01_river_island.jpg')
    place_photo(img, d, photo, 720, CONTENT_TOP, 480, 340, caption='橘子洲 · 湘江')
    d.text((720, CONTENT_TOP + 355), '↑ 毛泽东重游之地', fill=GRAY, font=F_SMALL)
...
def slide_duli():
    ...
    d.rectangle([(MARGIN, y), (680, y + 180)], fill=LIGHT_BG, outline=LIGHT_GRAY)
    ly = y + 10
    ...
    # 右侧：橘子洲实景
    photo = load_img('01_river_island.jpg')
    place_photo(img, d, photo, 720, CONTENT_TOP, 480, 380, caption='橘子洲实景 · 湘江北去')
...
# ═══════════════════════════════════════════════════
#  新增：6页意象可视化（费曼学习法——看图即懂）
# ═══════════════════════════════════════════════════

def slide_imagery_wanshan():
    """14. 意象可视化：万山红遍"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '万山红遍', page_num=next_num())
    photo = load_img('02_autumn_mountains.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='秋日群山红叶')
    # 右侧文字
    tx = 760
    d.text((tx, 130), '"看万山红遍"', fill=FROST, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=FROST, width=2)
    y = 190
    points = [
        ('"万"', '群山连绵，不止一座'),
        ('"红遍"', '秋色浓烈，漫山尽染'),
...
def slide_imagery_clin():
    """15. 意象可视化：层林尽染"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '层林尽染', page_num=next_num())
    photo = load_img('03_autumn_forest.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='层叠秋林色彩')
    tx = 760
    d.text((tx, 130), '"层林尽染"', fill=FROST, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=FROST, width=2)
    y = 190
    points = [
        ('"层"', '树林层层叠叠，有纵深'),
    ...
    save_slide(img, next_num(), 'img_clin')

def slide_imagery_manjiang():
    """16. 意象可视化：漫江碧透"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '漫江碧透', page_num=next_num())
    photo = load_img('04_green_river.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='清澈碧绿江水')
    tx = 760
    d.text((tx, 130), '"漫江碧透"', fill=XIANG, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=XIANG, width=2)
    y = 190
    points = [
        ('"漫"', '满江、整条江，无处不在'),
        ('"碧透"', '绿到能看穿水底'),
...
def slide_imagery_baige():
    """17. 意象可视化：百舸争流"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '百舸争流', page_num=next_num())
    photo = load_img('05_boats.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='江面船只竞发')
    tx = 760
    d.text((tx, 130), '"百舸争流"', fill=ACCENT, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=ACCENT, width=2)
    y = 190
    points = [
        ('"百"', '虚指，形容船多'),
        ('"舸"', '大船'),
...
def slide_imagery_yingji():
    """18. 意象可视化：鹰击长空"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '鹰击长空', page_num=next_num())
    photo = load_img('06_eagle.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='雄鹰翱翔长空')
    tx = 760
    d.text((tx, 130), '"鹰击长空"', fill=FROST, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=FROST, width=2)
    y = 190
    points = [
        ('"击"', '搏击、冲击——不是普通的飞'),
...
def slide_imagery_yuxiang():
    """19. 意象可视化：鱼翔浅底"""
    img, d = new_slide()
    draw_header(d, '意象可视化', '鱼翔浅底', page_num=next_num())
    photo = load_img('07_fish.jpg')
    place_photo(img, d, photo, 60, 110, 660, 500, caption='清澈水中的游鱼')
    tx = 760
    d.text((tx, 130), '"鱼翔浅底"', fill=XIANG, font=F_POEM_LG)
    d.line([(tx, 168), (tx + 400, 168)], fill=XIANG, width=2)
    y = 190
    points = [
        ('"翔"', '本指鸟飞——鱼如鸟般轻快'),
...
def slide_lianzi_1():
    ...
    # 下方放两张小图
    photo_eagle = load_img('06_eagle.jpg')
    photo_fish = load_img('07_fish.jpg')
    place_photo(img, d, photo_eagle, 80, y + 30, 280, 160, caption='鹰击长空')
    place_photo(img, d, photo_fish, 400, y + 30, 280, 160, caption='鱼翔浅底')
    # 右侧文字
    d.text((720, y + 50), '对比看图：', fill=ACCENT, font=F_BODY_B)
    d.text((720, y + 80), '鹰在天空搏击——', fill=INK, font=F_BODY)
    d.text((720, y + 105), '  那是"击"的力量', fill=FROST, font=F_BODY)
    d.text((720, y + 135), '鱼在水中飞翔——', fill=INK, font=F_BODY)
    d.text((720, y + 160), '  那是"翔"的自由', fill=XIANG, font=F_BODY)
...
def slide_p2_cover():
    ...
    if photo:
        ...
        img.paste(cropped, (0, 0))
        overlay = Image.new('RGB', (W, H), INK)
        blended = Image.blend(cropped, overlay, 0.5)
        img.paste(blended, (0, 0))
        d = ImageDraw.Draw(img)
    else:
        d.rectangle([(0, 0), (W, H)], fill=INK)
    d.text((MARGIN, 100), '沁园春·长沙', fill=WHITE, font=load_font('msyhbd.ttc', 72))
    d.text((MARGIN + d.textlength('沁园春·长沙', load_font('msyhbd.ttc', 72)) + 30, 120), '毛泽东', fill=(200,210,220), font=F_TITLE)
    d.rectangle([(MARGIN, 210), (MARGIN + 160, 248)], fill=XIANG)
    d.text((MARGIN + 20, 218), '第二课时', fill=WHITE, font=F_BODY_B)
    d.text((MARGIN, 340), '品析下阕 · 情景交融 · 拓展对比', fill=(200,210,220), font=F_SUB)
...
def slide_zhongliu():
    ...
    # 左侧：激流照片
    photo = load_img('08_waves.jpg')
    place_photo(img, d, photo, 60, 110, 520, 500, caption='中流击水 · 激流勇进')
    # 右侧文字
    tx = 620
    d.text((tx, 130), '"曾记否，到中流击水，浪遏飞舟？"', fill=XIANG, font=F_POEM)
    y = 175
    d.text((tx, y), '最后三句蕴含怎样的感情？', fill=INK, font=F_SUB)
    y += 38
    answers = [
        '内容：回忆当年在湘江游泳，不畏风浪',
        '修辞："浪遏飞舟"——夸张手法',
...
def main():
    print('Rendering classroom PPT v3 (text + images balanced)...')

    slides = [
        # 第一课时
        slide_cover,              # 1 封面(秋景照片)
        slide_objectives,         # 2 学习目标
        slide_leadin_autumn,      # 3 导入
        slide_author,             # 4 知人论世(橘子洲照片)
        slide_background,         # 5 写作背景
        slide_wenti,              # 6 文体知识
        slide_cipai,              # 7 词牌
        slide_zhengyin,           # 8 正字音
        slide_dujiepai,           # 9 朗读节拍
        slide_full_poem,          # 10 全词齐读
        slide_four_pictures,      # 11 四幅图
        slide_duli,               # 12 独立寒秋(橘子洲照片)
        slide_qiujing_kan,        # 13 "看"字领起
        # 意象可视化（6页新增——费曼学习法）
        slide_imagery_wanshan,    # 14 万山红遍(照片)
        slide_imagery_clin,       # 15 层林尽染(照片)
        slide_imagery_manjiang,   # 16 漫江碧透(照片)
        slide_imagery_baige,      # 17 百舸争流(照片)
        slide_imagery_yingji,     # 18 鹰击长空(照片)
        slide_imagery_yuxiang,    # 19 鱼翔浅底(照片)
        # 教学分析（文字表格）
        slide_imagery_table,      # 20 意象分析表
        slide_color_action,       # 21 色彩动态美
        slide_lianzi_1,           # 22 炼字击vs翔(鹰鱼照片)
        slide_lianzi_2,           # 23 炼字竞
        slide_transition,         # 24 过渡谁主沉浮
        slide_summary_1,          # 25 第一课时小结
        # 第二课时
        slide_p2_cover,           # 26 第二课时封面(秋景照片)
        slide_review,             # 27 回顾
        slide_xiaque_intro,       # 28 下阕引入
        slide_tongxue_image,      # 29 同学少年形象
        slide_lianzi_xia,         # 30 下阕炼字
        slide_diangu,             # 31 典故
        slide_zhongliu,           # 32 中流击水(激流照片)
        slide_qingjing,           # 33 情景交融
        slide_contrast,           # 34 对比阅读
        slide_theme,              # 35 主题归纳
        slide_blackboard,         # 36 板书设计
        slide_exercises,          # 37 课堂练习
        slide_homework,           # 38 课后作业
        slide_ending,             # 39 结语
    ]
    ...
    pdf_path = os.path.join(OUT, '沁园春长沙-课堂版PPT-v3.pdf')
    images[0].save(pdf_path, 'PDF', resolution=150, save_all=True, append_images=images[1:])

    desktop = os.path.expanduser('~/Desktop')
    import shutil
    shutil.copy(pdf_path, os.path.join(desktop, '沁园春长沙-课堂版PPT-v3.pdf'))
